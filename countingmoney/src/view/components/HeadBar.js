...
var HeadBar = (function (_super) {
    ...
    __proto.initSelf = function () {
        ...
        bg.loadImage("texture/game/bg_head_1.png");
        ...
        txt.scale(0.54,0.6);
        txt.pivotY = 18;
        txt.pos(this.width * 0.25 - 35, this.height / 2);
        this.addChild(txt);
        this.leftTxt = txt;
        ...
        txt1.scale(0.54,0.6);
        txt1.pivotY = 18;
        txt1.pos(this.width * 0.75 - 125, this.height / 2);
        this.addChild(txt1);
        this.rightTxt = txt1;

        var dollar = new Laya.Sprite();
        dollar.loadImage("texture/game/font/dollar.png");
        dollar.pivot(dollar.width/2,dollar.height/2);
        dollar.scale(0.7,0.7)
        dollar.pos(this.width/2 - 194,this.height / 2+1);
        this.addChild(dollar);
        this.leftDollar = dollar;

        var dollar1 = new Laya.Sprite();
        dollar1.loadImage("texture/game/font/dollar.png");
        dollar1.pivot(dollar1.width/2,dollar1.height/2);
        dollar1.scale(0.7,0.7)
        dollar1.pos(this.width/2 + 194,this.height / 2+1);
        this.addChild(dollar1);
        this.rightDollar = dollar1;
    };

    __proto.setNum = function(loc, num){
        if(loc == 0){
            this.leftTxt.text = "" + num;
            this.leftDollar.x = this.width/2 - 194 + (this.leftTxt.text.length - 1) * 17;
        }
        else{
            this.rightTxt.text = "" + num;
            this.rightDollar.x = this.width/2 + 194 - (this.rightTxt.text.length - 1) * 17;
        }
    }
