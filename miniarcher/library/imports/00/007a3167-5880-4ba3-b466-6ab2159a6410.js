"use strict";
cc._RF.push(module, '007a3FnWIBLo7RmarIVmmQQ', 'OverView');
// script/view/OverView.js
...
cc.Class({
    ...
    init: function init() {
        ...
        var no = gameInstance.gameView.modelType == 0 ? this.noArrow : this.noTime;
        no.stopAllActions();
        no.runAction(cc.sequence(cc.scaleTo(.1, .9).easing(cc.easeSineOut()), cc.callFunc(function () {
            no.runAction(cc.repeatForever(cc.sequence(cc.scaleTo(.2, 1.1).easing(cc.easeSineInOut()), cc.scaleTo(.2, .9).easing(cc.easeSineInOut()))));
        }.bind(this))));
