"use strict";
cc._RF.push(module, 'eea4a9sG95N+KQMZSKL0sWP', 'Target');
// script/view/Target.js
...
cc.Class({
    ...
    explode: function explode() {
        ...
        if (this.skinType == 2) {
            ...
            var pos = this.head.convertToWorldSpaceAR(cc.v2(0, 0));
    ...
    move: function move() {
        ...
        var offset = this.moveY / this.body.height;
        this.body.runAction(cc.sequence(cc.scaleTo(1 / this.speed, 1, 1 + this.moveDir * offset).easing(cc.easeSineOut()), cc.callFunc(function () {
            this.body.runAction(cc.repeatForever(cc.sequence(cc.scaleTo(2 / this.speed, 1, 1 - this.moveDir * offset).easing(cc.easeSineInOut()), cc.scaleTo(2 / this.speed, 1, 1 + this.moveDir * offset).easing(cc.easeSineInOut()))));
        }.bind(this))));
    },
    ...
    updateMask: function updateMask() {
        var pos = this.head.convertToWorldSpaceAR(cc.v2(0, 0));
        var mask = gameInstance.gameView.targetsMask[this.index];
        mask.position = mask.parent.convertToNodeSpaceAR(pos);
        mask.active = true;
    },
    setHeadPos: function setHeadPos() {
    ...
    rock: function rock() {
        this.rocking = true;
        this.body.parent.runAction(cc.sequence(cc.rotateBy(.2, -4 / 2).easing(cc.easeSineOut()), cc.rotateBy(.2, 7 / 2).easing(cc.easeSineInOut()), cc.rotateBy(.2, -5 / 2).easing(cc.easeSineInOut()), cc.rotateBy(.2, 3 / 2).easing(cc.easeSineInOut()), cc.rotateBy(.2, -1 / 2).easing(cc.easeSineIn()), cc.callFunc(function () {
            this.rocking = false;
        }.bind(this))));
    }
});
